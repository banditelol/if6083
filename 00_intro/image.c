...
float clamp_coor(int val, int lo, int hi) {
...
float get_pixel(image im, int x, int y, int c){
    x = clamp_coor(x, 0, im.w-1);
    y = clamp_coor(y, 0, im.h-1);
    c = clamp_coor(c, 0, im.c-1);
    return im.data[x + y*im.w + c*im.w*im.h];
}

void set_pixel(image im, int x, int y, int c, float v){
    if ( x != clamp_coor(x, 0, im.w-1) ) return;
    if ( y != clamp_coor(y, 0, im.h-1) ) return;
    if ( c != clamp_coor(c, 0, im.c-1) ) return;
